...
# add by wangbowen for v2xscenes 98.wangbowen@sjtu.edu.cn
def project_world_objects_v2xscenes(object_dict,
                          output_dict,
                          lidar_pose,
                          lidar_range,
                          order,
                          enlarge_z=False):
    """
    Project objects from world coordinates to LiDAR coordinate system.

    Parameters
    ----------
    object_dict : dict
        Dictionary containing objects with location, angle, center, extent
    output_dict : dict
        Output dictionary: key=object id, value=bbox [x,y,z,l,w,h,yaw]
    lidar_pose : list
        LiDAR pose in world coordinates [x,y,z,roll,yaw,pitch]
    lidar_range : list
        Filter range [minx,miny,minz,maxx,maxy,maxz]
    order : str
        Bbox dimension order: 'lwh' or 'hwl'
    enlarge_z : bool, optional
        Expand Z-axis range by ±10 for filtering
    """
    for object_id, object_content in object_dict.items():
        ...
        # Construct object pose in world coordinates
        object_pose = [location[0] + center[0],
                       location[1] + center[1],
                       location[2] + center[2],
                       rotation[0], rotation[1], rotation[2]]
        ...
        # Create 3D bounding box corners and transform to LiDAR coordinates
        bbx = create_bbx(extent).T
        bbx = np.r_[bbx, [np.ones(bbx.shape[1])]]  # Homogeneous coordinates
        bbx_lidar = np.dot(object2lidar, bbx).T
        bbx_lidar = np.expand_dims(bbx_lidar[:, :3], 0)
        bbx_lidar = corner_to_center(bbx_lidar, order=order)

        # Simplified representation using original location/dimensions
        x, y, z = location
        h, w, l = extent
        bbx_lidar = np.array([[x, y, z, 2*h, 2*w, 2*l, rotation[2]*np.pi/180]])

        # Expand Z range if requested
        if enlarge_z:
            lidar_range = copy.deepcopy(lidar_range)
            lidar_range[2] -= 10
            lidar_range[5] += 10

        # Filter objects outside LiDAR range
