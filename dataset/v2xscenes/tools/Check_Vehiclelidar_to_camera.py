...
camera_name = { 
                'BL': 'back_left_camera',
                'BR': 'back_right_camera',
                'FL': 'front_left_camera',
                'FR': 'front_right_camera',
                'L': 'left_camera',
                'R': 'right_camera'
                }
...
for num, sensor in enumerate(map_dict[f'{time_stamp}'][f'{time_stamp}']):
    if str(sensor['port']) == 'middle':
        pcd_path = dataset_dir + f"/{file_name}" + f"/veh_lidar/middle/{sensor['source_time']}.pcd" 
        points = filter_point_cloud_by_xyz(pcd_path, x_min, x_max, y_min, y_max, z_min, z_max)
        # point_cloud = o3d.io.read_point_cloud(pcd_path)
        # points = np.asarray(point_cloud.points)  
    if str(sensor['port']) == camera_name[ports]:
        image_path = dataset_dir + f"/{file_name}" + f"/veh_camera/{camera_name[ports]}/{sensor['source_time']}.jpg" 
        image = cv2.imread(image_path)

with open(dataset_dir + f'/Calibration/Vehiclelidar_to_camera/Lidar_camera{ports}_T/calib.json') as f:
    calib = json.load(f)
...
# 保存图片
import os
output_path = dataset_dir + f"/{file_name}/visualization/Check_Vehiclelidar_to_camera/{str(time_stamp)}/middle_{ports}_{time_stamp}.jpg"
if not os.path.exists(dataset_dir + f"/{file_name}/visualization/Check_Vehiclelidar_to_camera/{str(time_stamp)}/"):
    os.makedirs(dataset_dir + f"/{file_name}/visualization/Check_Vehiclelidar_to_camera/{str(time_stamp)}/")
